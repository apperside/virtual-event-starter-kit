...
export default function ArtistSection({ artist }: Props) {
  return (
    <>
      <Link href="/speakers">
      ...
      <div key={artist.name} className={styles.container}>
      ...
      {/* {artist.talk && (
        <div className={styles['talk-details']}>
          <h3 className={styles['socials-header']}>{artist.talk.title}</h3>
          <p>{artist.talk.description}</p>
        </div>
      )} */}
    </>
  );
